...
class BasicLayout extends React.PureComponent {
  constructor(props) {
    super(props);
    const {routes} = props.route,routeKey = '/home/home',tabName = '起始页'; // routeKey 为设置首页设置 试试 '/dashboard/analysis' 或其他key值
  ...
  render() {
    ...
    const operations = (
      <Dropdown overlay={menu} >
        <a className="ant-dropdown-link" href="#" style={{marginRight:5}}>
          <Icon type="menu" style={{marginBottom: 5, color: 'black', fontWeight: 700}} />
        </a>
      </Dropdown>
    );
            ...
            {hidenAntTabs ?
              ...
              >
                {this.state.tabList.map((item,index) => (
                  <TabPane id={'my-tab:' + index} tab={item.tab} key={item.key} closable={item.closable}>
